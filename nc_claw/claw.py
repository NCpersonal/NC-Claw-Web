#!/usr/bin/env python3
"""
Claw v0.6.10 — Terminal AI Assistant
...
def parse_commands(text):
    ...
    # ── Pass 1: Line-by-line multi-line commands ──
    i = 0
    while i < len(lines):
        stripped = lines[i].strip()

        # //write path
        wm = re.match(r'^(?://)?write\s+(.+)$', stripped)
        if wm:
        ...
        # //python ... //end
        pm = re.match(r'^(?://)?python\s*$', stripped)
        if pm:
            code_lines = []
            i += 1
            while i < len(lines):
                ...
                i += 1
            cmds.append({"type": "python", "args": "\n".join(code_lines)})
            i += 1
            continue

        # //exec ... //end
        em = re.match(r'^(?://)?exec\s*$', stripped)
        if em:
            cmd_lines = []
            i += 1
            while i < len(lines):
    ...
    # ── Pass 2: Single-line commands (scan every line) ──
    for line in lines:
        stripped = line.strip()
        sm = re.match(r'^(?://)?(\w+)\b\s*(.*?)\s*$', stripped)
        if sm:
            ct = sm.group(1).lower()
            args = sm.group(2).strip().strip('`"\'*').rstrip('.,;:!?')
            if ct not in CMD_TYPES:
                continue
            if ct == "write":
                continue
            if ct in ("python", "exec") and not args:
                continue
            if not args and ct not in ("info", "time", "ip"):
                continue
            # Deduplicate: skip if already captured in pass 1
            dup = False
            for existing in cmds:
                if existing["type"] == ct and existing.get("args") == args:
                    dup = True
                    break
            if not dup:
                cmds.append({"type": ct, "args": args})
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            self.send_json({"status": "ok", "version": "0.6.10", "model": config["model"],
        ...
        if path == "/api":
            self.send_json({"name": "Claw Gateway", "version": "0.6.10",
...
def print_status():
    ...
    print()
    print("  " + sep)
    print("  {}{}{} Claw v0.6.10".format(C.B, C.CYN, C.R))
    print("  " + sep)
    print()
...
def cmd_help():
    ...
    a = L.append
    a("")
    a("  {}{}\U0001F43E Claw v0.6.10 — Commands{}".format(C.CYN, B, R))
    a("")
